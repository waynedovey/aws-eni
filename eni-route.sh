...
VPC_CIDR_URI_ETH0="http://169.254.169.254/latest/meta-data/network/interfaces/macs/${ETH0_MAC}/vpc-ipv4-cidr-block"
log "Metadata location for vpc ipv4 range: ${VPC_CIDR_URI_ETH0}"
...
if [ $? -ne 0 ]; then
   ...
   log "Retrieved VPC CIDR range ${VPC_CIDR_RANGE_ETH0} from meta-data."
fi

VPC_URI_ETH0_IP="http://169.254.169.254/latest/meta-data/network/interfaces/macs/${ETH0_MAC}/local-ipv4s"
log "Metadata location for vpc ipv4 add: ${VPC_URI_ETH0_IP}"

VPC_ETH0_IP=$(curl --retry 3 --silent --fail ${VPC_URI_ETH0_IP})
if [ $? -ne 0 ]; then
   log "Unable to retrive VPC IP from meta-data, using null instead"
   VPC_ETH0_IP=""
else
   log "Retrieved VPC IP ${VPC_ETH0_IP} from meta-data."
fi

VPC_URI_ETH1_IP="http://169.254.169.254/latest/meta-data/network/interfaces/macs/${ETH0_MAC}/local-ipv4s"
log "Metadata location for vpc ipv4 add: ${VPC_URI_ETH1_IP}"
...
if [ $? -ne 0 ]; then
   ...
   log "Retrieved VPC IP ${VPC_ETH1_IP} from meta-data."
fi

VPC_CIDR_URI_ETH1="http://169.254.169.254/latest/meta-data/network/interfaces/macs/${ETH1_MAC}/vpc-ipv4-cidr-block"
log "Metadata location for vpc ipv4 range: ${VPC_CIDR_URI_ETH1}"
...
if [ $? -ne 0 ]; then
   ...
   log "Retrieved VPC CIDR range ${VPC_CIDR_RANGE_ETH1} from meta-data."
fi

ip route add ${VPC_CIDR_RANGE_ETH0} dev eth0 tab 1
ip route add ${VPC_CIDR_RANGE_ETH1} dev eth1 tab 2

ETH0_ROUTE=$(route -n|grep "UG"|grep -v "UGH" | grep eth0|cut -f 10 -d " ")
ETH1_ROUTE=$(route -n|grep "UG"|grep -v "UGH" | grep eth1|cut -f 10 -d " ")
